...
**F. Help-centre statements** — `artifacts/preventivo-ai/src/data/help-articles.ts` (live at `/help/`): the articles describe the legal mechanics in plain language (cooling-off, holdback, lien periods, CASL). Ask them to skim for anything that reads as legal advice.
