...
| **A-1 Fatture SDI** | scelta e contratto intermediario (Openapi default; preventivo A-Cube per confronto), onboarding delega/codice destinatario guidato, XML FatturaPA dal motore fatture v2, stati SdI via webhook, conservazione, note di credito, bollo virtuale + F24 trimestrale, ciclo passivo in ricezione | 2–3 sett. |
| **A-2 Motore fiscale forfettario** | `lib/fiscal/rules/2026.ts` versionato, onboarding fiscale, calcolo live, "quanto mettere via", monitor soglia con pipeline, simulatore; ogni numero spiegato; test golden-file contro casi verificati dal commercialista | 2 sett. |
| **A-3 Scadenzario + F24** | calendario personalizzato, F24 precompilati (imposte, INPS, bollo), promemoria email/WhatsApp, quietanze | 1 sett. |
