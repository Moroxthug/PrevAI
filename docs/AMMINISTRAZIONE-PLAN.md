...
| **A-4 Prima nota + chiusura d'anno** ✅ codice (2026-09-23) — resta D6 | prima nota che **legge** incassi, costi e versamenti dalle tabelle di sempre (una tabella nuova solo per i movimenti senza posto), import CSV delle banche italiane e OFX con riconciliazione suggerita e mai automatica (tranne "abbina i sicuri", che non crea nulla), utile netto dopo le tasse di competenza con i costi presunti dal coefficiente accanto a quelli reali, chiusura d'anno come fotografia firmata che non blocca i dati, prospetto dei righi LM/RR (regola **F20**, `non_revisionata`), PDF + CSV, guida al fai-da-te, link in sola lettura per il commercialista con scadenza, revoca e registro degli accessi. Dettagli operativi in `RUNBOOKS.md` §9 | 2 sett. |
| **A-5 Pricing & lancio** ✅ codice (2026-09-23) — resta D5 | add-on come secondo abbonamento Stripe con prezzi cercati per lookup key e controllati contro la configurazione, paywall in app, landing `/amministrazione/` con test di prezzo a tre varianti (hash dell'impresa o `?v=` della campagna), `ops:addon-beta` per gli utenti beta, tre articoli SEO con i numeri del motore. Nome, prezzi e stato dell'offerta in un punto solo (`lib/config/src/offerta.ts`); lo stato effettivo si **calcola** dai prerequisiti: `interesse` con D5, `vendita` anche con D6 e D8, livello gratuito solo con D6. Il livello gratuito è `fiscal_engine`; la parte a pagamento è la nuova `admin_suite` (F24, prima nota, chiusura, link commercialista). Dettagli operativi in `RUNBOOKS.md` §10 | 1 sett. |
| **A-6 Commercialista nel loop** ✅ codice (2026-09-23) — restano D9 e D11 | professionisti verificati da PrevAI (Albo, Entratel, polizza RC ricontrollata a ogni richiesta) con convenzione firmata in app; incarico digitale con lettera (oggetto, compenso, polizza, antiriciclaggio), informativa IA (L. 132/2025 art. 13) e informativa privacy del professionista (titolare autonomo, niente contitolarità); pratica legata all'**impronta della chiusura d'anno** (consegna → revisione → bozza → conferma del cliente → protocollo → ricevuta), rifiutata se i numeri cambiano dopo la conferma; chat solo fra le due parti, senza IA; ogni accesso del professionista visibile al cliente; compensi per pratica maturati alla ricevuta. D9 decide solo l'assegnazione (con D9 aperta la fa lo staff a mano); il servizio resta dietro il flag `accountant_service` e l'offerta in `bozza` finché D9, D11 e un professionista operativo non ci sono. Dettagli operativi in `RUNBOOKS.md` §11 | 3–4 sett. + negoziazione |
