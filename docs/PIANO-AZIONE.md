...
| 4 | **V2-3** Prova generale su `prevai-staging` da dump reale; migrazioni additive; v1 e v2 entrambe funzionanti sul DB migrato | V2 §4 | 2–3 g | ⬜ | | | **← PROSSIMA**. Fase che protegge i dati |
...
## Come iniziare la prossima sessione (V2-3)

Apri una sessione nella cartella `C:\Users\Admin\Downloads\PrevAI (2)\PrevAI`, `git checkout v2`, e scrivi: **"Leggi docs/PIANO-AZIONE.md ed esegui la fase V2-3"**. Dettagli in `PREVAI-V2-PLAN.md` §4 V2-3. Stato al 2026-09-21: V2-2 chiusa sul branch `v2` (non ancora pushato dopo la sotto-tappa b — pushare `v2` aggiorna solo la *preview* Vercel; `main` resta v1 in produzione). Prima di V2-3 serve: progetto Supabase `prevai-staging` (o schema separato) su cui caricare il dump baseline (`docs/RUNBOOKS.md`), le env v2 elencate in `docs/ENV-INVENTORY.md` (sezione QuoteAI) per la preview, e la decisione **D2** (finestra di cutover) resta per V2-5.
