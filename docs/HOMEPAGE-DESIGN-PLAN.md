...
Read `artifacts/preventivo-ai/src/pages/home.tsx` and `artifacts/preventivo-ai/src/index.css` end to end. Three concrete, fixable problems, not a vague "make it nicer":
...
Replace the Scale-AI-derived tokens in `artifacts/preventivo-ai/src/index.css` (`:root` block, currently commented "scala Scale AI"). Proposed palette, grounded in the same restraint principle as §3, but distinct from Intuit's own trade dress (we're not copying their black+green, we're copying their *discipline*):
...
- Add the `simple-icons` npm package to `artifacts/preventivo-ai` (MIT-licensed SVG path data for official brand marks) — covers Google, Gmail, Meta/Facebook, WhatsApp, Microsoft/Outlook, Stripe, QuickBooks out of the box.
