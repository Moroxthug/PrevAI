...
**Frontend** (`artifacts/preventivo-ai/src/`): `components/payment-schedule-editor.tsx`, `components/payment-schedule-card.tsx` (quote detail sidebar), `pages/dashboard/settings-business-tab.tsx` (Settings → Business), `components/notifications-bell.tsx` (sidebar, 60 s polling), `lib/plans.ts`, `lib/payment-schedule.ts`; EN + FR strings.
...
- Vercel deploys are made from the local machine with `npx vercel deploy --prod --yes` (project `preventivo-ai`, Hobby plan) — GitHub pushes do NOT auto-deploy.
