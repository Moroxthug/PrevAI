...
| `VITE_POSTHOG_HOST` | — |  | defaults to https://eu.i.posthog.com | preventivo-ai/src/lib/analytics.ts | recommended for launch, not set |
| `VITE_POSTHOG_KEY` | — |  | browser analytics (loaded on first interaction) | preventivo-ai/src/App.tsx, preventivo-ai/src/lib/analytics.ts | recommended for launch, not set |
| `VITE_SENTRY_DSN` | — |  | browser error tracking (same DSN is fine) | preventivo-ai/src/lib/error-tracking.ts | recommended for launch, not set |
