...
export default function PublicQuotePage() {
  ...
  async function handleAccept() {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) {
        setError(data.error || "Impossibile registrare l'accettazione. Riprova.");
        return;
      }
      setQuote(data.quote);
    } catch {
  ...
  if (notFound || !quote) {
    return (
      <div className="min-h-[70vh] flex flex-col items-center justify-center text-center px-4">
        ...
        <h1 className="text-lg font-semibold text-gray-900">Preventivo non disponibile</h1>
        <p className="text-sm text-gray-500 mt-1 max-w-sm">
          Il link potrebbe essere scaduto oppure il preventivo non è ancora stato reso disponibile dal professionista.
        </p>
      </div>
    );
  }
    ...
    <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-10 max-w-2xl">
      <div className="text-center mb-8">
        <div className="text-xs font-bold text-violet-600 uppercase tracking-wider mb-2">
          {quote.companySnapshot?.companyName || "Preventivo"}
        </div>
      ...
      {isAccepted ? (
        <Card className="border-emerald-200 bg-emerald-50">
          <CardContent className="p-5 sm:p-6 flex items-start gap-3">
            ...
            <div>
              <p className="text-sm font-semibold text-emerald-800">Preventivo accettato</p>
              <p className="text-xs text-emerald-700 mt-0.5">
                Confermato da {quote.acceptedByName}
                {quote.acceptedAt && (
                  <> il {format(new Date(quote.acceptedAt), "d MMMM yyyy 'alle' HH:mm", { locale: it })}</>
                )}.
              </p>
            </div>
          </CardContent>
        ...
        <Card className="border-gray-100">
          <CardContent className="p-5 sm:p-6">
            <p className="text-sm font-semibold text-gray-900 mb-1">Accetta questo preventivo</p>
            <p className="text-xs text-gray-500 mb-4">
              Scrivi il tuo nome e cognome per confermare che accetti condizioni e importo indicati sopra.
            </p>
            <div className="space-y-3">
              <div>
                <Label htmlFor="nomeConferma" className="text-xs text-gray-500">Nome e cognome</Label>
                <Input
                  id="nomeConferma"
                  value={nomeConferma}
      ...
      <p className="text-center text-[11px] text-gray-300 mt-8">
        Preventivo generato con prevai.it
      </p>
    </div>
  );
}
