...
async function fetchOfficialSourceSnippet(url: string): Promise<{ ok: boolean; snippet: string }> {
  // INCENTIVES_SOURCE_FETCH=off: nessuna richiesta verso i siti ufficiali (suite e2e, ambienti chiusi).
  if (process.env.INCENTIVES_SOURCE_FETCH === "off") return { ok: false, snippet: "" };
  try {
...
Restituisci SOLO un JSON valido nel seguente formato:
{
  ...
  try {
    ...
    // Senza esito AI si aggiorna solo lastCheckedAt (ramo sotto): in v1 l'array vuoto
    // di default saltava l'aggiornamento e i bandi restavano "mai controllati".
    aiResult = { updatedStatus: undefined, riepilogoScansione: "Verifica AI non disponibile: aggiornato solo il timestamp di controllo." };
  }
