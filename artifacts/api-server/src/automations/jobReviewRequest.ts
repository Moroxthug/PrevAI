...
registerAutomation("job.review_request_due", async (run) => {
  ...
  const result = await sendJobReviewRequest({ client, profile, reviewUrl: profile.googleReviewUrl, secondaryReviewUrl: profile.secondaryReviewUrl, whatsappTemplateName });
  if (!result.ok) {
